using System;
using System.Data.CRUD;
using MySql.Data.MySqlClient;

namespace GG.SSO.Entity.Table.Sso
{
	[DataClassCRUD("sso","clientproperties")]
	public class ClientProperties : Entity<ClientProperties>
	{
		[DataMemberCRUD("Id", MySqlDbType.Int32, 10, true, true)]
		public int Id { get; set; }

		[DataMemberCRUD("Clients_Id", MySqlDbType.Int32, 10, true)]
		public int Clients_Id { get; set; }

		[DataMemberCRUD("Key", MySqlDbType.VarChar, 250)]
		public string Key { get; set; }

		[DataMemberCRUD("Value", MySqlDbType.VarChar, 2000)]
		public string Value { get; set; }

		public ClientProperties()
		{}

		public ClientProperties(int id,int clients_Id,string key,string value)
		{
			Id = id;
			Clients_Id = clients_Id;
			Key = key;
			Value = value;
		}
	}
}
